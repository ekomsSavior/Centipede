...
// stageBinary loads this process's own binary into memory so it can be
// dropped on remote hosts. Safe to call repeatedly.
func (r *Replicator) stageBinary() error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if len(r.binary) > 0 {
		return nil
