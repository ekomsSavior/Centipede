...
// Operator auth: attach the stored token to every request; on 401 ask once.
const _fetch = window.fetch;
async function c2fetch(url, opts) {
    opts = opts || {};
    opts.headers = Object.assign({}, opts.headers);
    const t = localStorage.getItem('c2token');
    if (t) opts.headers['X-C2-Token'] = t;
    const r = await _fetch(url, opts);
    if (r.status === 401 && !String(url).includes('/login')) {
        const tok = prompt('Enter operator token:');
        if (tok) {
            localStorage.setItem('c2token', tok);
            await _fetch('/login?token=' + encodeURIComponent(tok));
            return c2fetch(url, opts);
        }
    }
    return r;
}
window.fetch = c2fetch;
...
function handleWSMessage(data) {
    switch(data.t) {
        ...
        case 'bot_result':
            ...
            break;
        case 'bot_gone':
            addActivity('bot', `Bot offline: ${data.bid.slice(0,8)}`);
            loadStats(); loadBots();
            break;
    }
}
